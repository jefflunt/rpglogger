class WorldObjectProperty < ActiveRecord::Base
  ...
  default_scope includes(:section_property).order("section_properties.sort_order ASC").where("section_properties.id = world_object_properties.section_property_id AND section_properties.archived_at IS NULL")
