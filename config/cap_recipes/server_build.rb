namespace :build do
  ...
  task :install_s3ql_dependencies, roles: :web do
    step_output(server_step+=1, total_steps, "Installing S3QL package dependencies...")
    run "#{sudo} apt-get -y install s3ql"
  end
  
  desc "Install RVM in single user mode"
  task :install_rvm_in_single_user_mode, roles: :web do
    step_output(server_step+=1, total_steps, "Installing RVM in single-user mode...")
  ...
  task :move_profile_contents_to_bashrc_file, roles: :web do
    step_output(server_step+=1, total_steps, "Moving .bash_profile contents to .bashrc...")
  ...
  task :install_rvm_zlib_package, roles: :web do
    step_output(server_step+=1, total_steps, "Installing RVM zlib package...")
  ...
  task :install_ruby192_into_rvm, roles: :web do
    step_output(server_step+=1, total_steps, "Installing Ruby 1.9.2. Compiling from source (SLOW)...")
    run "rvm install ruby-1.9.2-p320"
  end
  
  desc "Install 'bundler' gem"
  task :install_bundler_gem, roles: :web do
    step_output(server_step+=1, total_steps, "Installing 'bundler' gem...")
