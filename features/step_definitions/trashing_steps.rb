...
Then /^the LogBook "(.*?)" should NOT be marked as deleted$/ do |log_book_title|
  LogBook.find_by_title(log_book_title).deleted_at.should be nil
end
...
Then /^the Section "(.*?)" in "(.*?)" should NOT be marked as deleted$/ do |section_name, log_book_title|
  LogBook.find_by_title(log_book_title).sections.find_by_name(section_name).deleted_at.should be nil
end
...
Then /^the WorldObject "(.*?)" in "(.*?)" in "(.*?)" should NOT be marked as deleted$/ do |world_object_name, section_name, log_book_title|
  LogBook.find_by_title(log_book_title).sections.find_by_name(section_name).world_objects.find_by_name(world_object_name).deleted_at.should be nil
end